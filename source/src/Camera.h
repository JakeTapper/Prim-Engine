...
public:
	...
	void update(glm::vec3* position) {
		...
		//If there is no camera position given, assume that the position is at (0,0,0)
		/*if (!position) {
			glm::vec3 cameraTarget = glm::vec3(0.0f, -1.0f, -5.0f);
			...
			cameraPos = pos + glm::vec3(0, .5, 0) - (cameraFront * glm::vec3(7));
			glm::vec3 cameraTarget = glm::vec3(0.0f, 0.0f, 0.0f);
			glm::vec3 cameraDirection = glm::normalize(pos - cameraTarget);
			glm::vec3 up = glm::vec3(0.0f, 1.0f, 0.0f);
			glm::vec3 cameraRight = glm::normalize(glm::cross(up, cameraDirection));

			glm::mat4 view = glm::lookAt(cameraPos, pos + cameraFront, cameraUp);
		//}
